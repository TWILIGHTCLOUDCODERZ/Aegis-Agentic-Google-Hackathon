    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="page-wrap">
          {activeNav === 'Aegis AI Intelligence' ? <CommandCenter transactions={transactions} blocked={blocked} selected={selected} onSelect={setSelected} onInvestigate={(t) => { setSelected(t); setActiveNav('Investigations'); }} onNavigate={setActiveNav} streaming={streaming} onToggle={() => setStreaming(!streaming)} /> : activeNav === 'Architecture' ? <ArchitecturePage /> : activeNav === 'Customer App' ? <FraudJourney embedded /> : activeNav === 'Customer 360' ? <Customer360 /> : activeNav === 'Compliance / SAR' ? <ComplianceSAR /> : activeNav === 'Network Graph' ? <NetworkGraph /> : <PlaceholderPage activeNav={activeNav} transactions={transactions} selected={selected} onSelect={setSelected} onNavigate={setActiveNav} />}
        </div>
      </main>
    </div>
  );
...
function CommandCenter({ transactions, blocked, selected, onInvestigate, onNavigate, streaming, onToggle }: { transactions: Transaction[]; blocked: Transaction[]; selected: Transaction; onSelect: (transaction: Transaction) => void; onInvestigate: (t: Transaction) => void; onNavigate: (page: string) => void; streaming: boolean; onToggle: () => void }) {
  return <>
    ...
    <section className="kpi-grid"><Kpi icon={Activity} label="Transactions monitored" value="2,481,905" delta="+12.8%" positive accent="blue" onClick={() => onNavigate('Network Graph')} /><Kpi icon={ShieldAlert} label="Fraud blocked" value={number(184290)} meta="21 incidents" accent="red" onClick={() => onNavigate('Investigations')} /><Kpi icon={Target} label="False declines prevented" value="68.4%" delta="+62% vs rules-only" positive accent="green" onClick={() => onNavigate('Investigations')} /><Kpi icon={Clock3} label="Avg. resolution time" value="18s" delta="down from 12m" positive accent="amber" onClick={() => onNavigate('Investigations')} /><Kpi icon={Users} label="Analyst hours saved" value="1,248h" meta="this week" accent="blue" onClick={() => onNavigate('Investigations')} /><Kpi icon={Shield} label="Money protected" value="$4.82M" delta="+18.6%" positive accent="green" onClick={() => onNavigate('Compliance / SAR')} /></section>
    <div className="dashboard-grid"><section className="panel feed-panel"><div className="panel-heading"><div><div className="section-kicker"><span className="green-dot" /> LIVE FEED</div><h2>Transaction activity</h2></div><button className="filter-button"><ListFilter size={15} /> Filter</button></div><div className="feed-table-head"><span>Transaction</span><span>Customer</span><span>Merchant</span><span>Location</span><span>Decision</span></div><div className="feed-list">{transactions.map((transaction) => <TransactionRow key={transaction.id} transaction={transaction} onClick={() => onInvestigate(transaction)} />)}</div><button className="view-all" onClick={() => onNavigate('Investigations')}>View all activity <ArrowUpRight size={15} /></button></section><aside className="panel alert-panel"><div className="panel-heading"><div><div className="section-kicker red-kicker"><span className="red-dot" /> NEEDS ATTENTION</div><h2>Alert queue <span className="count-pill">{Math.max(3, blocked.length + 2)}</span></h2></div><button className="more-button">•••</button></div><div className="alert-list">{[selected, ...blocked, transactions[3]].filter((value, index, array) => value && array.findIndex((item) => item.id === value.id) === index).slice(0, 4).map((transaction) => <button className="alert-item" key={transaction.id} onClick={() => onInvestigate(transaction)}><div className={`risk-score ${transaction.risk > 85 ? 'critical' : 'high'}`}>{transaction.risk}</div><div className="alert-copy"><strong>{transaction.customer}</strong><span>{transaction.id} · {transaction.city}</span></div><div className="alert-value">{number(transaction.amount)}<small>{transaction.decision}</small></div><ChevronRight size={15} /></button>)}</div><button className="view-all" onClick={() => onNavigate('Investigations')}>Open investigation queue <ArrowUpRight size={15} /></button></aside></div>
    ...
    <section className="memory-strip"><div className="memory-icon"><BrainCircuit size={22} /></div><div><span className="section-kicker blue-kicker">Aegis memory active</span><p><strong>Recalled:</strong> Maya Patel confirmed Dubai travel on Mar 3. Pattern matches — customer was not re-challenged.</p></div><button onClick={() => onInvestigate(initialTransactions[0])}>View evidence <ArrowUpRight size={15} /></button></section>
  </>;
}

function Kpi({ icon: Icon, label, value, delta, meta, positive, accent, onClick }: { icon: typeof Activity; label: string; value: string; delta?: string; meta?: string; positive?: boolean; accent: string; onClick?: () => void }) { return <div className={`kpi-card ${accent}`} onClick={onClick} style={onClick ? { cursor: 'pointer' } : undefined}><div className="kpi-top"><span className="kpi-icon"><Icon size={16} /></span><span className="kpi-label">{label}</span><ArrowUpRight size={15} className="kpi-arrow" /></div><div className="kpi-value">{value}</div><div className={`kpi-meta ${positive ? 'positive' : ''}`}>{positive && <ArrowUpRight size={13} />}{delta || meta}</div></div>; }
    ...
    <div className="investigation-panel panel">
      <div className="investigation-top">
        <div>
          ...
          <h2 style={{ cursor: 'pointer' }} onClick={() => onNavigate('Customer 360')} title="View Customer 360">{transaction.customer}</h2>
      ...
      {status === 'done' && <div className="analyst-actions"><button className="action-primary" onClick={() => setActionMsg('✓ Approved by analyst — decision recorded in the case.')}><Check size={16} /> Approve</button><button onClick={() => setActionMsg('Decision overridden — routed for secondary review.')}><ShieldAlert size={16} /> Override</button><button onClick={() => setActionMsg('Escalated to the fraud investigation team.')}><ArrowUpRight size={16} /> Escalate</button><button onClick={() => onNavigate('Customer 360')}><BrainCircuit size={16} /> Add to memory</button></div>}
